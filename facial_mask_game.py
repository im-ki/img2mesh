...
def compute_visuals(input_img, coeffs, renderer, device='cpu'):
    with torch.no_grad():

        pred_mask, pred_face = renderer(coeffs['id'], coeffs['exp'], coeffs['angle'], coeffs['trans'], coeffs['tex'], coeffs['gamma'])
...
def load_facial_mask(path):
    mask = {}
    filenames = [i for i in os.listdir(path) if i[-3:] == 'npy']
    for i in filenames:
        mask_key = i.split('.')[0]
        mask_value = np.load(os.path.join(path, i))
...
def load_model():
    ...
    renderer = prepare_for_render(focal=1015, center=112., device=device, camera_distance=10., init_lit=np.array([0.8, 0, 0, 0, 0, 0, 0, 0, 0]), recenter=True, znear=5., zfar=15., bfm_file='./BFM/BFM_model_front.mat')
    ...
    def Run(im):
        ...
        with torch.no_grad():
            ...
            output_im = compute_visuals(im_tensor, pred_coeffs, renderer)
